...
def variational_layer(tparams, state_below, options, prefix='variational', mask=None, training=True, nsteps = None,**kwargs):
    ...
    if training:
        generate_mu = tensor.dot(h_, W_dec_mu) + W_dec_mu_b
    else:
        generate_mu = tensor.dot(h_[None,:], W_dec_mu) + W_dec_mu_b
...
def param_init_variational_gru(options, params, prefix='variational_gru', nin=None, dim=None, dimv=None):
    ...
    Z = numpy.concatenate([norm_weight(options['dim_pi'],dim, dim), norm_weight(options['dim_pi'], dim)], axis=1)
    ...
    Zx = norm_weight(options['dim_pi'], dim)
...
def variational_gru_layer(tparams, state_below, options, prefix='variational_gru', mask=None, one_step=False,init_state=None, **kwargs):
    ...
    def _step_slice(m_, x, x_, xx_, h_, gx, U, Ux, Z, Zx, i0, i1, i2, v0, v1, v2, v3, pi, n):
        preact = tensor.dot(h_, U)
        preact += x_
        tparams_i = [i0, i1, i2]
        tparams_v = [v0, v1, v2, v3]
        #compute attened image
        ctxpi = get_layer('image_attention')[1](tparams_i,[pi,h_],options=options)

        preact_z = tensor.dot(ctxpi,Z)
        preact += preact_z

        #compute z
        g_x = get_layer('variational')[1](tparams_v,[ctxpi,h_,x],options,mask=m_,nsteps=n, training=True)
        ...
        preactx = preactx * r
        preactx = preactx + xx_ + tensor.dot(ctxpi, Zx)
    ...
    def _step_slice_sample(m_, x_, xx_, h_, gx, U, Ux, Z, Zx, i0, i1, i2, v0, v1, v2, v3, pi, n):
        preact = tensor.dot(h_, U)
        preact += x_
        ...
        #compute z
        g_x = get_layer('variational')[1](tparams_v,[ctxpi,h_],options,mask=m_,nsteps=1, training=False)
    ...
    nn = x.shape[1]
    if not one_step:
        init_states = [h0, tensor.alloc(0., n_samples, options['n_words_src'])]
...
def build_model(tparams, options, training=True):
    ...
    g_xs = get_layer('variational_gru')[1](tparams, [pi3, emb], options, mask=x_mask)
    ...
    return trng,use_noise,x,x_mask,pi,opt_ret,cost,g_xs
...
def gen_sample(tparams, f_init, f_next, pi, options, trng=None, k=1, maxlen=30,stochastic=True, argmax=False):
...
def adam(lr, tparams, grads, inp, cost, g_xs, x, beta1=0.9, beta2=0.999, e=1e-8):

    gshared = [theano.shared(p.get_value() * 0., name='%s_grad' % k)
               for k, p in tparams.iteritems()]
    gsup = [(gs, g) for gs, g in zip(gshared, grads)]

    f_grad_shared = theano.function(inp, [cost, g_xs, x], updates=gsup, profile=profile)
...
def train(dim_word=100,  # word vector dimensionality
    ...
    f_grad_shared, f_update = eval(optimizer)(lr, tparams, grads, inps, cost, g_xs, x)
    print 'Done'
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, pi in train:
            ...
            cost, g_xs, xs = f_grad_shared(x, x_mask, pi)
            print "Truth"
            for ww in xs[:,0]:
                if ww == 0:
                    break
            ...
            if numpy.mod(uidx, dispFreq) == 0:
                print 'Epoch ', eidx, 'Update ', uidx, 'Cost ', cost, 'UD ', ud, 'kl-cost ', 'likelihood', cost
