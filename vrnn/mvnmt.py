...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'variational_gru':('param_init_variatinal_gru','variational_gru_layer'),
          'image_attention':('param_init_image_attention','image_attention_layer'),
          'image': ('param_init_image', 'image_layer')
          }
...
def param_init_image(options, params, prefix='image', nin=None, nout=None, ortho=True):
    if nin is None:
        nin = options['dim_pi']
    if nout is None:
...
def param_init_image_attention(options, params, prefix="image_attention", nin_pi=None, nin_h=None, nout=None, ortho=True):
    ...
    if nin_h is None:
        nin_h = options['dim']
    if nout is None:
        nout = options["dim_pi_ctx"]

    params[_p(prefix, 'W')] = norm_weight(nin, nout, scale=0.01)
    params[_p(prefix, 'U')] = norm_weight(nin, nout, scale=0.01)
    params[_p(prefix, 'b')] = numpy.zeros((nout,)).astype('float32')
...
def image_attention_layer(tparams, state_below, options, prefix='image_attention', **kwargs):
    ...
    p_i = tensor.dot(state_below[0],tparams[_p(prefix, 'U')])
    p_h = tensor.dot(state_below[1]. tparams[_p(prefix, 'W')])
    p_ih = p_i + p_h[:,None,:]
    e = tensor.dot(tensor.tanh(p_ih),tparams[_p(prefix, 'b')])
    alpha = tensor.exp(e) / tensor.sum(tensor.exp(e))
    ctx = T.sum(state_below[0] * alpha[:,:,None], axis=1)
    return ctx
    
#variation layer
def param_init_variational(options, params, prefix='variational', nin=None, dim=None, dimz=None):
    if nin is None:
        nin = optios['dim_proj']
    if dim is None:
    ...
    W_pri = numpy.concatenate([norm_weight(dimctxpi,dimv),norm_weight(dim,dimv)])
    ...
    W_post = numpy.concatenate([norm_weight(dimctxpi,dimv),norm_weight(dim,dimv),norm_weight(nin,dimv)])
    params[_p(prefix, 'W_post_pi')] = W_post
    ...
    W_dec_x = norm_weight(nin,options['dim_words_src'])
    params[_p(prefix, 'W_dec_x')] = W_dec_x
    params[_p(prefix, 'W_dec_x_b')] = numpy.zeros((options['dim_words_src'],)).astype('float32')

    return params

def variational_layer(tparams, state_below, options, prefix='variational', mask=None, **kwargs):
    ctxpi = state_below[0]
    h_ = state_below[1]
    x = state_below[2]
    nsteps = h_.shape[0]
    ...
    #prior
    pri_1 = tensor.dot(concatenate([ctxpi,h_,],axis=1),tparams[_p(prefix, 'W_pri')]) + tparams[_p(prefix, 'W_pri_b')]
    pri_mu = tensor.dot(pri_1, tparams[_p(prefix, 'W_pri_mu')]) + tparams[_p(prefix, 'W_pri_mu_b')]
    pri_sigma = tensor.nnet.softplus(tensor.dot(pri_1, tparams[_p(prefix, 'W_pri_mu')]) + tparams[_p(prefix, 'W_pri_mu_b')])
    
    #posterior
    if training:
        post_1 = tensor.dot(concatenate([ctxpi,h_,x], axis=1))
        post_mu = tensor.dot(post_1, tparams[_p(prefix, 'W_post_mu')]) + tparams[_p(prefix, 'W_post_mu_b')]
        post_sigma = tensor.nnet.softplus(tensor.dot(post_1, tparams[_p(prefix, 'W_post_sigma')]) + tparams[_p(prefix, 'W_post_sigma_b')])
    #KL computing
    kl_cost = 0
    epsilon = 10**(-8)
    if training:
    ...
    encoded_z = tensor.dot(sample_z, tparams[_p(prefix, 'W_z')])
    generate_mu = tensor.dot(encoded_z, tparams[_p(prefix, 'W_dec_mu')]) + tparams[_p(prefix, 'W_dec_mu_b')]
    #generate_sigma = tensor.dot(encoded_z, tparams[_p(prefix, 'W_dec_sigma')]) + tparams[_p(prefix, 'W_dec_sigma_b')]
    generated_x = tensor.nnet.softmax(tensor.dot(generate_mu, tparams[_p(prefix, 'W_dec_x')])+tparams[_p(prefix, 'W_dec_x_b')])
    return kl_cost, sample_z, generated_x

#GRU layer
def param_init_variational_gru(options, params, prefix='vrnn', nin=None, dim=None, dimz=None):
    if nin is None:
        nin = optios['dim_proj']
    if dim is None:
        dim = options['dim_proj']
    ...
    Z = numpy.concatenate([norm_weight(dimz, dim), norm_weight(dimz, dim)], axis=1)
    ...
    Zx = norm_weight(dimz, dim)
    params[_p(prefix, 'Zx')] = Zx
    
    return params

def variational_gru_layer(tparams, state_below, options, prefix='variational_gru', mask=None, **kwargs):
    #pis (batch, 196, 512)
    #xs  (seq, batch, nin)
    pis = state_below[0]
    xs = state_below[1]
    ...
    dim = tparams[_p(prrefix, 'Ux')].shape[1]

    if mask is None:
        mask = tensor.alloc(1., xs.shape[0], 1)
    ...
    state_below_ = tensor.dot(xs, tparams[_p(prefix, 'W')]) + \
        tparams[_p(prefix, 'b')]
    # input to compute the hidden state proposal
    state_belowx = tensor.dot(xs, tparams[_p(prefix, 'Wx')]) + \
        tparams[_p(prefix, 'bx')]

    # step function to be used by scan
    # arguments    | sequences |outputs-info| non-seqs
    def _step_slice(m_, x, x_, xx_, h_, kl, gx, U, Ux, Z, Zx, pi):
        preact = tensor.dot(h_, U)
        preact += x_

        #compute attened image
        ctxpi = image_attention_layer(tparams,[pi,h_],options=options)
        
        #compute z
        kl, z, g_x = variational_layer(tparams,[ctxpi,h_,x],options,mask)
    ...
    seqs = [mask, xs, state_below_, state_belowx]
    init_states = [tensor.alloc(0., n_samples, dim),tensor.alloc(0.,1),tensor.alloc(0., n_samples, options['dim_words_src'])]
    _step = _step_slice
...
def init_params_vrnn(options):
    ...
    params = get_layer('variational')[0](options, params, prefix='variational',nin=options['dim_word'],dim=options['dim'],dimz=options['dimz'])

    params = get_layer('image_attention')[0](options, params, nin_pi=options['dim_pi'], nin_h=options['dim'], nout=options['dimctxpi'])

    params = get_layer('variational_gru')[0](options, params, prefix='vrnn', nin=options['dim_word'], dim=options['dim'], dimz=options['dimz'])

    return params
    
def build_model_lm(tparams, options, training=True):
    opt_ret = dict()
    ...
    kl_cost, g_xs = get_layer('variational_gru')[1](tparams, [pi,emb], options, mask=x_mask)
    ...
    return x,x_mask,pi,kl_cost,g_xs,cost,trng,use_noise,opt_ret
