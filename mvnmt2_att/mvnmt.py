...
def param_init_image(options, params, prefix='image', nin=None, nin_h=None, nout=None, ortho=True):
    if nin is None:
        nin = options['dim_pi']
    if nin_h is None:
        nin_h = options['dim'] * 2
    ...
    params[_p(prefix, 'W')] = norm_weight(nin_h, nout, scale=0.01, ortho=ortho)
    params[_p(prefix, 'U')] = norm_weight(nin, nout, scale=0.01, ortho=ortho)
    params[_p(prefix, 'b')] = numpy.zeros((nout,)).astype('float32')

    return params

def image_layer(tparams, state_below, options, prefix='image', **kwargs):
    #state_below[0] is image and state_below[1] is mean hidden states of x.
...
def param_init_variation(options, params, prefix='variation',
                         ...
                         nin_nonlin=None, dim_nonlin=None,dimv=None, dim_pi=None, dim_enc_z=None):
    if nin is None:
        nin = options['dim']
    if dim is None:
        dim = options['dim']
...
def param_init_gru_cond(options, params, prefix='gru_cond',
                        nin=None, dim=None, dimctx=None,dim_enc_z=None,
