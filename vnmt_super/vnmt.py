...
def build_model(tparams, options, training=True):
    ...
    projx = get_layer(options['encoder'])[1](tparams, emb, options,
                                            prefix='encoder_x',
                                            mask=x_mask)
    projxr = get_layer(options['encoder'])[1](tparams, embr, options,
                                             prefix='encoder_r_x',
                                             mask=xr_mask)
    ...
    ctx_x = concatenate([projx[0], projxr[0][::-1]], axis=projx[0].ndim-1)
    ctx_y = concatenate([projy[0], projry[0][::-1]], axis=projy[0].ndim-1)
    
    # mean of the context (across time) will be used to initialize decoder rnn
    ctx_mean = (ctx_x * x_mask[:, :, None]).sum(0) / x_mask.sum(0)[:, None]
