...
def param_init_variation(options, params, prefix='variation',
    ...
    W_pri_pi = norm_weight(dimctx,dimv)
...
def variation_layer(tparams, ctx_means, options, prefix='variation', ctx_y_means=None, pic=None, mask=None,training=True, **kwargs):
    ...
    pri_h = tensor.tanh(tensor.dot(ctx_means, tparams[_p(prefix, 'W_pri_pi')]) + tparams[_p(prefix, 'W_pri_pi_b')])
    if training:
        post_h = tensor.tanh(tensor.dot(concatenate([ctx_means, ctx_y_means],axis=1), tparams[_p(prefix, 'W_post_pi')]) + tparams[_p(prefix, 'W_post_pi_b')])
...
def build_model(tparams, options, training=True):
    ...
    projx = get_layer(options['encoder'])[1](tparams, emb, options,
                                            prefix='encoder',
                                            mask=x_mask)
    cmu = concatenate([pic, emb], axis=0)
    proj = get_layer(options['encoder'])[1](tparams, cmu, options,
                                            prefix='encoder',
                                            mask=cmu_mask)
    ...
    projrx = get_layer(options['encoder'])[1](tparams, embr, options,
                                            prefix='encoder_r',
                                            mask=x_mask)
    projr = get_layer(options['encoder'])[1](tparams, cmur, options,
                                             prefix='encoder_r',
                                             mask=cmu_mask_r)
    ...
    ctx_mean = (ctx * cmu_mask[:, :, None]).sum(0) / cmu_mask.sum(0)[:, None]
    ctx_x_mean = (ctx_x * x_mask[:, :, None]).sum(0) / x_mask.sum(0)[:, None]
...
def build_sampler(tparams, options, trng, use_noise):
    ...
    sample_z,_ = get_layer('variation')[1](tparams, ctx_x_mean, options, prefix='variation', training=False)
