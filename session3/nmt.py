...
def build_model(tparams, options):
    ...
    emby= emb.reshape([n_timesteps, n_samples, options['dim_word']])
    projy = get_layer(options['encoder'])[1](tparams, emb, options,
                                            prefix='encoder',
                                            mask=y_mask)

    #word embedding for backward rnn (target)
    emby = tparams['Wemb_y'][yr.flatten()]
    emby = embr.reshape([n_timesteps, n_samples, options['dim_word']])
    projry = get_layer(options['encoder'])[1](tparams, embr, options,
                                             prefix='encoder_r',
                                             mask=yr_mask)
    ...
    cost = (cost * y_mask).sum(0) + kl_cost
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=True, training=True):
    probs = []
    ...
    for x, y in iterator:
        ...
        pprobs = f_log_probs(x, x_mask, y, y_mask,training)
        for pp in pprobs:
            probs.append(pp)
